...
const {test,loginUser,getProfile,registerUser,verifyMail, createCampaign,PasswordReset,NewPassword, stripeIntegration,logsout,fetchFundraise, ProfileDetails} = require('../controllers/authControllers')
...
router.get('/profile',getProfile,(req,res)=>{
    return res.json({Status:"Success",name:req.name, email:req.email, userId:req.id})
})
